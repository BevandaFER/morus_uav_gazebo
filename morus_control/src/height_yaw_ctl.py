...
class HeightControl:
    ...
    def __init__(self):
        ...
        rospy.Subscriber('pose_with_covariance',
        ...
        self.pub_pid_z = rospy.Publisher('pid_z', PIDController,
                queue_size=1)
        self.pub_pid_vz = rospy.Publisher('pid_vz', PIDController,
                queue_size=1)
        self.pub_pid_yaw_rate = rospy.Publisher('pid_yaw_rate',
                PIDController, queue_size=1)
        self.mot_ref_pub = rospy.Publisher('mot_vel_ref', Float32,
                queue_size=1)
    ...
    def run(self):
        ...
        print ''
        print 'Hello from height control'
        ...
        while not rospy.is_shutdown():
            ...
            if not self.start_flag:
                ...
                a = 0.1
                self.z_ref_filt = (1 - a) * self.z_ref_filt + a \
                    * self.z_sp
                vz_ref = self.pid_z.compute(self.z_ref_filt, self.z_mv,
                        dt)
                self.mot_speed = self.mot_speed_hover \
                    + self.pid_vz.compute(vz_ref, self.vz_mv, dt)

                self.dwz = self.pid_yaw_rate.compute(self.euler_sp.z,
                ...
                if self.gm_attitude_ctl == 0:
                    ...
                    # Publisherlish motor velocities
